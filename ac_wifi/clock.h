...
inline uint8_t time_update_take() {
  ...
  time_update = 0;
  interrupts(); // codex修改:
                // 主循环一次性取走并清空事件位，避免与回调并发时丢失分钟/小时事件
  return flags;
}
void wifi_status();
uint8_t wifi_fail = 0;
...
void sec() {
  ...
  if (now.tm_sec >= 60) {
    now.tm_sec -= 60;
    now.tm_min++;
    datamins[now.tm_min % 60] =
        0.0; // codex修改:
             // 分钟切换时立即清空新分钟槽位，避免主循环稍后再清零把新分钟前几秒的峰值抹掉
  ...
  if (switch_change_time > 60 && switch_change_time < 65 &&
      sets.on_off != digitalRead(SSR)) { // switch状态改变1分钟后， 保存
    deferred_action_set(
        DEFER_SAVE_SWITCH); // codex修改: 定时回调里只置延后动作，避免在 Ticker
                            // 上下文直接访问 SPIFFS 和蜂鸣器
  }
  switch_change_time++;
